/* Patched SignalRGB.c for SignalRGB Protocol */ //Replace original SignalRGB.c
...
/* VIA path:
 * Returning true means we consumed the packet entirely.
 * Returning false allows VIA core to process non-SignalRGB packets.
 */
bool via_command_kb(uint8_t *data, uint8_t length) {
    if (srgb_raw_hid_rx(data, length)) {
        return true; // handled by SignalRGB module
    }
    return false; // forward to VIA so custom/standard VIA commands still work
}
#else
/* Non-VIA path:
 * Forward any packet NOT claimed by SignalRGB to the keyboard's normal
 * raw_hid handler chain.  Some keyboards override raw_hid_receive_kb,
 * others only implement raw_hid_receive_user, so we probe both weak
 * symbols before dropping the packet.
 */
extern __attribute__((weak)) void raw_hid_receive_kb(uint8_t *data, uint8_t length);
extern __attribute__((weak)) void raw_hid_receive_user(uint8_t *data, uint8_t length);

static void forward_non_signalrgb(uint8_t *data, uint8_t length) {
    if (raw_hid_receive_kb) {
        raw_hid_receive_kb(data, length);
        return;
    }
    if (raw_hid_receive_user) {
...
void raw_hid_receive(uint8_t *data, uint8_t length) {
    if (srgb_raw_hid_rx(data, length)) {
        return; // consumed by SignalRGB
    }
    // Not a SignalRGB command: pass through to keyboard/user handlers.
    forward_non_signalrgb(data, length);
}
#endif
